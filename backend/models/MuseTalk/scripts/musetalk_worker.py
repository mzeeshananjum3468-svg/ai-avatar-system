...
Protocol (spoken over stdin/stdout, one JSON object per line):
  ...
  Per job:
    stdin  <- {"image": "<avatar photo path>", "audio": "<wav/mp3 path>",
               ...
               "bbox_shift": <int, optional, default 0>,
  ...
  "bbox_shift" moves the top edge of the detected face crop up (negative) or
  down (positive), within roughly [-9, 9] px for a typical portrait — it's a
  manual per-avatar tuning knob (see MuseTalk's own bbox_shift UI), not
  something this worker can compute automatically. When it differs from
  whatever value is baked into coord_cache, the cache is treated as stale and
  the face crop/latents/mask are recomputed for the new value (see
  Worker._prepare_avatar).
...
class Worker:
    ...
    def _prepare_avatar_image(self, image_path: str, bbox_shift: int = 0) -> dict:
        frame = cv2.imread(image_path)
        if frame is None:
            raise RuntimeError(f"Could not read avatar image: {image_path}")

        coord_list, _ = get_landmark_and_bbox([image_path], bbox_shift)
        bbox = coord_list[0]
        if bbox == coord_placeholder:
            raise RuntimeError("No face detected in avatar image")

        return {"mode": "image", "frames": [self._build_frame_material(frame, bbox)], "bbox_shift": bbox_shift}

    def _prepare_avatar_video(self, video_path: str, bbox_shift: int = 0) -> dict:
        ...
        try:
            ...
            coord_list, _ = get_landmark_and_bbox(frame_paths, bbox_shift)
            ...
            return {"mode": "video", "frames": materials, "bbox_shift": bbox_shift}
        finally:
            shutil.rmtree(tmp_dir, ignore_errors=True)

    def _prepare_avatar(self, image_path: str, coord_cache: str | None, bbox_shift: int = 0) -> dict:
        if coord_cache and os.path.exists(coord_cache):
            try:
                ...
                if isinstance(cached, dict) and "frames" in cached and cached["frames"]:
                    if cached.get("bbox_shift", 0) == bbox_shift:
                        return cached
                    print(f"[musetalk_worker] cache at {coord_cache} was built with "
                          f"bbox_shift={cached.get('bbox_shift', 0)}, requested "
                          f"{bbox_shift} — recomputing")
                else:
        ...
        material = (
            self._prepare_avatar_video(image_path, bbox_shift)
            if is_video
            else self._prepare_avatar_image(image_path, bbox_shift)
        )
    ...
    def infer(
        ...
        material = self._prepare_avatar(image_path, coord_cache, bbox_shift)

        # On a coord_cache miss, _prepare_avatar just ran face landmark
        # detection, whose FaceAlignment ctor (musetalk/utils/face_detection/
        # api.py) unconditionally sets this *process-global* torch flag to
        # True as a side effect. That leaks into the VAE/UNet convs below,
        # which see a different frame count (hence a different last-batch
        # shape) on essentially every call, since chunk length is TTS-driven.
        # Under benchmark=True, cuDNN autotunes and caches the fastest algo
        # per exact shape the first time it's seen — and that first pass for
        # a brand-new shape has been observed to occasionally emit corrupted
        # output (surfaced downstream as a malformed face-crop array feeding
        # blending.py's Image.fromarray). Pin it back off: this workload's
        # shapes vary too much per call for benchmark mode to pay for itself
        # anyway, so there's no upside to leaving it on.
        ...
        try:
            ...
            _run_silent(
                [
                    ...
                    "-vcodec", "libx264",
                    # Source frames inherit the avatar image's raw dimensions, which
                    # can be odd (e.g. 447x447) — libx264 requires even width/height.
